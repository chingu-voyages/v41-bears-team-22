...
class User {
	...
	static async register({
		...
		const result = await db.query(
			`INSERT INTO users (first_name, last_name, username, email, password, is_admin)
                VALUES($1, $2, $3, $4, $5, $6)
                RETURNING first_name AS "firstName", last_name AS "lastName", username, email, is_Admin AS "isAdmin"`,
	 ...
	 *
	 * Returns { username, firstName, lastName, email, isAdmin, appliedRoles }
	 *          where appliedRoles is {
	 *                                  companyName,
	 *                                  role,
	 *                                  location,
	 *                                  jobpostlink
	...
	static async get(username) {
		...
		const userApplications = await db.query(
			`SELECT a.company_name AS "companyName", a.role, a.location, a.jobpostlink
			 FROM applications AS a 
			 WHERE a.username = $1`,
			[username]
		);

		const roles = userApplications.rows;
		user.appliedRoles = roles;
		return user;
	}
	...
	/** Delete a given username from database
	 *
	 * Return undefined
	 */
